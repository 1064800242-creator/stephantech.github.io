            ...
            <div :key="activeStage">
              ...
              <div v-else class="replan-stage">
                ...
                <div v-if="quizScore === 3 && rewritePassed" class="transfer-box">
                  ...
                  <p v-if="transferChecked" class="transfer-result">{{ transferPassed ? '迁移验证通过：目标结构在新题中稳定使用。' : '检测到同类问题复发：系统会把你带回对应微训练。' }}</p>
                </div>
...
.transfer-box{margin-top:26px;padding:22px;border:1px solid rgba(255,255,255,.1);border-radius:15px;background:#0c1915}.transfer-box h4{font-size:22px;margin:8px 0 14px}.transfer-prompt{padding:15px;border:1px solid rgba(88,223,170,.22);border-radius:10px;background:#091512}.transfer-prompt b{color:#62e0aa;font-size:11px;letter-spacing:.1em}.transfer-prompt p{margin:8px 0 0;color:#c5d5ce;line-height:1.55}.transfer-instruction{color:#8aa097;line-height:1.6}.transfer-box textarea{width:100%;min-height:100px;box-sizing:border-box;padding:12px;border:1px solid #294138;border-radius:9px;background:#07110f;color:#e6f3ed;resize:vertical}.transfer-meta{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:12px}.transfer-meta span{font-size:11px;color:#6f867c}.transfer-meta button{border:0;background:#67e5af;color:#062218;border-radius:9px;padding:11px 15px;font-weight:800}.transfer-meta button:disabled{opacity:.35}.transfer-result{margin:12px 0 0;color:#69e2af;font-weight:700}
